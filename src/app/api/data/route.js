...
export async function GET(request) {
  try {
    ...
    // LOB-level RBAC enforcement
    if (session.allowedLobs && session.allowedLobs.length > 0) {
      const allowedLobs = new Set(session.allowedLobs.map(l => l.toLowerCase()));
      rows = rows.filter(r => {
        const lob = (r.LOB || '').toLowerCase();
        return allowedLobs.has(lob);
      });
    }
